 ...
 */
/**
 * Пункты, где нумерация CSV расходится с нумерацией в нормах.
 *
 * Приказ 812/пр делит п.104 «Капремонт и модернизация оборудования лифтов» на
 * две части, в CSV они названы '104' и '104доп', а в ГЭСНмр — '104.1' и '104.2'.
 * Сопоставление проверено по принадлежности каждой из 249 норм к Отделу
 * (второй сегмент кода): все 218 норм с 104.1 лежат в Отделах 01-04, все 31
 * норма с 104.2 — в Отделах 05-06, нарушений границы ноль. Это ровно то
 * деление, что записано в CSV: «кроме отделов 5,6» и «отделы 5,6».
 */
const ITEM_NO_ALIASES = {
  '104': '104.1',
  '104доп': '104.2',
};
...
export function normalizeNormKey(raw) {
  ...
  if (!s) return null;
  if (ITEM_NO_ALIASES[s]) s = ITEM_NO_ALIASES[s];
